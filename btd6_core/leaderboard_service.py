...
from api_raw_fetcher import ApiClient, fetch_bosses, fetch_races
...
def fetch_leaderboard_raw_data(client: ApiClient, leaderboard_type: str) -> dict[str, Any]:
    if leaderboard_type == "race":
        return {"races": fetch_races(client)}
    if leaderboard_type in {"boss-standard", "boss-elite"}:
...
def resolve_leaderboard(
    ...
    try:
        raw = fetch_leaderboard_raw_data(client, leaderboard_type)
